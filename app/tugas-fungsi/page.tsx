...
export const metadata = createPageMetadata({
  ...
  description:
    "Tugas dan fungsi Kepala Badan, Sekretariat, Bidang Pendataan, Penagihan, Pengawasan dan Pemeriksaan, Teknologi Informasi, serta kelompok jabatan fungsional.",
  path: "/tugas-fungsi",
  keywords: [
...
const departmentIcons: Record<string, ComponentType<SVGProps<SVGSVGElement>>> =
  {
...
function buildMergedData() {
  return data.tugas.map((tugas) => {
    const fungsiItem = data.fungsi.find(
      (fungsi) => fungsi.department === tugas.department
    );
    return {
  ...
  return (
    <article className="typography container mx-auto flex flex-col px-6">
      <section className="mt-16 flex flex-col">
        <h1 className="max-w-prose">Tugas & Fungsi</h1>
        <p className="max-w-prose text-muted-foreground">
          Tugas dan fungsi Badan Pendapatan Daerah Kabupaten Garut dalam
          mengelola urusan pendapatan daerah, pendataan dan penetapan,
          penagihan, pengawasan dan pemeriksaan, serta teknologi informasi.
        </p>
